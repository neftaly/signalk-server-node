...
module.exports = function (app, config) {
  ...
  strategy.deleteDevice = (config, uuid, callback) => {
    assertConfigImmutability()
    for (var i = config.devices.length - 1; i >= 0; i--) {
      if (config.devices[i].uuid == uuid) {
  ...
  strategy.updateDevice = (config, uuid, updates, callback) => {
    assertConfigImmutability()
    var device = config.devices.find(d => d.uuid == uuid)
  ...
  function getPrincipal (payload) {
    ...
    if (payload.id) {
      ...
      var device = options.devices.find(device => device.uuid == payload.device)
      if (device) {
        principal = {
  ...
  strategy.getAccessRequestsResponse = () => {
    return accessRequests.filter(r => r.active)
  }
  ...
  strategy.setAccessRequestStatus = (config, identifier, status, body, cb) => {
    var idx = accessRequests.findIndex(
      r => r.active && r.identifier == identifier
    )
    if (idx == -1) {
      cb(new Error('not found'))
      return
    ...
    if (status === 'approved') {
      if (request.uuid) {
        ...
        config.devices = config.devices.filter(d => d.uuid != identifier)
    ...
    if (request.completedCb) {
      request.completedCb(status, request.token)
    }
  ...
  strategy.requestAccess = (config, request, sourceIp, completedCb, cb) => {
    ...
    request.completedCb = completedCb
    ...
    if (request.uuid) {
      ...
      if (accessRequests.find(r => r.active && r.uuid == request.uuid)) {
        cb(
          new Error(
      ...
      */
      ...
      debug(
        `A device with IP ${request.ip} and UUID ${
          request.uuid

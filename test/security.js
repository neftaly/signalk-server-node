...
describe('Security', () => {
  ...
  it('Device access request and approval works', async function () {
    var result = await fetch(`${url}/access/requests`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        uuid: '1235-45653-343453',
    ...
    var result = await fetch(`${url}/access/requests/${requestJson.requestId}`)
    var json = await result.json()
    json.should.have.property('status')
    json.status.should.equal('PENDING')
    ...
    var json = await result.json()
    json.should.have.property('status')
    json.status.should.equal('APPROVED')
    ...
    json[0].uuid.should.equal('1235-45653-343453')
  ...
  it('Device access request and denial works', async function () {
    ...
    var json = await result.json()
    json.should.have.property('status')
    json.status.should.equal('DENIED')
